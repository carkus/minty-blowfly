class SubcomponentsController < ApplicationController
  ...
  def index
    ...
    @subcomponents = Subcomponent.all #where("owner_id = ?", params[:userid])
  ...
  private
    ...
    def subcomponent_params
      params.require(:subcomponent).permit(:behaviour, :refid, :ownerid, :editorid, :followerid, :views, :viewable, :active, :title, :subs, :description, :titleimage, :meat)
    end
end
